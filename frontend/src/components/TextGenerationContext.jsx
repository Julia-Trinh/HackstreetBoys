...
export const TextGenerationProvider = ({ children }) => {
    ...
    const generateText = async (difficulty = 1, numWords = 15, sentence = false, personnalized = false, focusLetters) => {
        try {
            ...
            MAKE SURE the result contains EXACTLY ${numWords} words. MAKE SURE the words are separated by a single space. MAKE SURE the words are gramatically correct. MAKE SURE the respone uses EITHER programming terms OR western terms. NEVER give the exact same response twice in a row. NEVER include words that contain apostrophes`;
